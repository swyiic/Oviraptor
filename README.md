...
一个面向授权资产管理场景的跨平台桌面端。界面使用 Vue 3，桌面壳和任务编排使用 Tauri 2 / Rust，数据保存在应用私有目录中的 SQLite 文件里；不要求系统预装 SQLite。
...
当前工作区已将 `probe_output_20260714` 的 39,975 条原始结果导入“`中国移动历史资产`”项目；应用会将相同端点软隔离，原始记录和源 CSV 均保留。
