...
pub fn create_sentinel_url_scan(
    ...
    let scan_mode = match scan_mode.as_deref() {
        Some(value @ ("quick" | "standard" | "deep")) => value.to_string(),
        _ => "standard".into(),
    };
...
const SENTINEL_SCAN_COLUMNS: &str = "id,project_id,project_name,status,current_checkpoint,task_path,previous_scan_id,llm_requests,input_tokens,output_tokens,cached_tokens,total_tokens,scan_type,task_name,source_path,skill_names,attempt_count,created_at,updated_at";
...
fn prepare_web_scan_retry(
    ...
    let retry_incomplete_only = status == "partial";
    // Old releases could place model/runtime failures in the fuse zone. They
    // are execution failures, not target risk, and must not block an in-place
    // retry after the model configuration has been corrected.
    let _ = connection.execute(
        "DELETE FROM sentinel_fuse_zone WHERE verdict='pending' AND trim(note)='' AND reason LIKE '模型 %' AND EXISTS (SELECT 1 FROM sentinel_targets t WHERE t.scan_id=?1 AND t.project_id=sentinel_fuse_zone.project_id AND lower(rtrim(trim(t.url),'/'))=sentinel_fuse_zone.normalized_url)",
        [scan_id],
    );
    ...
    let fuse_excluded: i64 = connection.query_row(
        "SELECT COUNT(*) FROM sentinel_targets t WHERE t.scan_id=?1 AND (?2=0 OR status NOT IN ('completed','recon_only','manual_review')) AND EXISTS (SELECT 1 FROM sentinel_fuse_zone f WHERE f.project_id=t.project_id AND f.normalized_url=lower(rtrim(trim(t.url),'/')))",
        params![scan_id, retry_incomplete_only as i64],
        |row| row.get(0),
    ).unwrap_or(0);
    let checkpoint = if fuse_excluded > 0 {
        format!("正在继续当前任务；复用已有前端证据，已排除熔断区中的 {fuse_excluded} 个 URL")
    } else {
        "正在继续当前任务；复用已有前端证据并重试未完成阶段".to_string()
    };
    ...
    transaction
        .execute(
            "UPDATE sentinel_targets SET status='queued',value_score=0,scan_mode='',routing_reason='',updated_at=datetime('now','localtime') WHERE scan_id=?1 AND (?2=0 OR status NOT IN ('completed','recon_only','manual_review')) AND NOT EXISTS (SELECT 1 FROM sentinel_fuse_zone f WHERE f.project_id=sentinel_targets.project_id AND f.normalized_url=lower(rtrim(trim(sentinel_targets.url),'/')))",
            params![scan_id, retry_incomplete_only as i64],
        )
        .map_err(|error| error.to_string())?;
    transaction
...
pub fn rescan_sentinel_scan(
    ...
    let (scan_type, status): (String, String) = connection
        .query_row(
            "SELECT s.scan_type,s.status FROM sentinel_scans s JOIN projects p ON p.id=s.project_id AND p.status='active' WHERE s.id=?1",
            [&scan_id],
...
fn collect_strix_trace(
    ...
    let run_dirs = trace_run_dirs(&task_path, scan_id);
    ...
    for run_dir in run_dirs {
        ...
        if let Ok(bytes) = fs::read(run_dir.join("run.json")) {
        ...
        let agents_path = run_dir.join(".state/agents.db");
...
pub fn list_strix_traces(state: State<AppState>) -> Result<Vec<StrixTraceSummary>, String> {
    ...
    Ok(ids
        .iter()
        .filter_map(|id| {
            collect_strix_trace(&connection, id, false)
                .ok()
                .map(|value| value.0)
        })
        .collect())
...
pub fn get_strix_trace(
    ...
    let connection = db::open(&state.db_path)?;
    let task_path = strix_trace_base(&connection, &scan_id)?.4;
    let (summary, events) = collect_strix_trace(&connection, &scan_id, true)?;
    ...
    if let Some(audit) = prompt_audit.as_mut() {
        ...
        if summary.exact_request_capture {
            ...
            audit.notice = "Oviraptor instruction 快照与本地 LLM Hook 捕获的最终模型请求均按原文保存在本机；逐请求内容位于下方调用时间线。".into();
        }
    }
    Ok(StrixTraceDetail {
        summary,
