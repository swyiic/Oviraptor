...
const newAuthSessionScope = () => globalThis.crypto.randomUUID();
...
function startAuthCapturePolling(session: BrowserAuthSession) {
  ...
  const timer = window.setInterval(async () => {
    if (authBusy.value && authBusy.value !== session.id) return;
...
const webPolicySummary = computed(() => {
  if (form.scanMode === "quick")
    return { contracts: 4, discovery: 1, verifiers: 1, chain: false };
  if (form.scanMode === "deep")
    return { contracts: 24, discovery: 3, verifiers: 3, chain: true };
  return { contracts: 12, discovery: 2, verifiers: 2, chain: false };
});
...
async function loadAuthSessions() {
  ...
  try {
    authSessions.value = await api.listBrowserAuthSessions(form.projectId, authSessionScopeId.value);
...
function toggleAuthSession(sessionId: string, selected: boolean) {
  if (selected) {
    if (form.authSessionIds.length >= 5) {
      emit("notify", "info", tr("单个任务最多比较 5 个独立登录身份", "A task can compare up to five isolated identities"));
      return;
    }
...
function onAuthSessionToggle(sessionId: string, event: Event) {
  toggleAuthSession(sessionId, (event.target as HTMLInputElement | null)?.checked === true);
}
function firstUrl() {
  return form.urls
    .split(/\r?\n|,/)
...
async function openLogin(session?: BrowserAuthSession) {
  ...
  try {
    const opened = await api.openBrowserAuthSession({
      ...
      entryUrl,
      draftScopeId: authSessionScopeId.value,
    });
    startAuthCapturePolling(opened);
    await loadAuthSessions();
    emit("notify", "info", tr("登录窗口已打开：登录成功并访问后台功能后，会自动保存身份、显示右上角成功提示并关闭窗口", "Login window opened. After a successful login and an authenticated feature request, the identity is saved automatically and the window closes."));
  } catch (error) {
    emit("notify", "error", String(error));
  } finally {
    authBusy.value = "";
...
async function finishLogin(session: BrowserAuthSession) {
  ...
  try {
    ...
    if (updated.status !== "valid") {
      emit("notify", "info", updated.lastError || tr("已捕获，但需要再访问一个登录后功能", "Captured, but visit an authenticated feature first"));
    }
  } catch (error) {
    emit("notify", "error", String(error));
  } finally {
...
async function start() {
  ...
  try {
    ...
    const scan = await api.startStrixWorkbenchScan({
      ...
      authSessionScopeId: authSessionScopeId.value,
...
onMounted(async () => {
  unlistenAuthSession = await listen<BrowserAuthSession>("browser-auth-session-updated", async (event) => {
    ...
    if (!session || session.projectId !== form.projectId || session.draftScopeId !== authSessionScopeId.value) return;
    await loadAuthSessions();
    if (session.status === "valid") {
      ...
      form.authSessionName = "";
      // The WebView is closed by the Rust finish command. Close the selector as
      // well so the success state is unambiguous and the parent can continue.
      showAuthSessionPicker.value = false;
      emit("notify", "success", tr(`登录成功，${session.name} 已保存身份`, `Login succeeded; ${session.name} identity saved`));
    }
  });
        ...
        <section class="panel workbench-form">
          ...
          <div class="workbench-fields">
            ...
            <label v-if="mode === 'web'" class="field span-two"
              ...
              ></textarea>
              <small class="helper">{{ tr("提交后立即进入 Web 自适应扫描；低价值目标会以确定性侦察正常收口并显示扫描完成。", "The task starts immediately; low-value targets finish normally with deterministic reconnaissance.") }}</small>
            </label>
            ...
            <section v-if="mode === 'web' || mode === 'greybox'" class="browser-auth-center span-two" :class="{ expanded: showAuthSessionPicker }">
              <header>
            ...
            <label class="field"
              ...
              ><input
                ...
                placeholder="—"
            /></label>
            <section v-if="mode === 'web'" class="effective-policy-card span-two">
              <header>
                <div>
                ...
                <em>{{ form.scanMode.toUpperCase() }}</em>
              </header>
              <dl>
                <div><dt>{{ tr("验证契约", "Contracts") }}</dt><dd>≤ {{ webPolicySummary.contracts }}</dd></div>
                <div><dt>{{ tr("定向发现", "Discovery") }}</dt><dd>{{ webPolicySummary.discovery }} {{ tr("轮", "passes") }}</dd></div>
                <div><dt>{{ tr("验证器", "Verifiers") }}</dt><dd>≤ {{ webPolicySummary.verifiers }}</dd></div>
                <div><dt>{{ tr("攻击链关联", "Attack chains") }}</dt><dd>{{ webPolicySummary.chain ? tr("启用", "On") : tr("仅单点证据", "Single findings") }}</dd></div>
              </dl>
              <p><Check :size="13" />{{ effectiveWebSkillNames.join(" + ") }}</p>
            ...
            <div v-if="mode !== 'web' || showAdvanced" class="field span-two">
              ...
              <div class="skill-choice-grid">
                <label
                  v-for="skill in skills.filter((item) => item.enabled && (mode !== 'web' || !item.builtin))"
                  :key="skill.id"
                ...
                >
                <div
                  v-if="!skills.some((item) => item.enabled && (mode !== 'web' || !item.builtin))"
                  class="empty-inline"
                >
                  {{
                    tr(
